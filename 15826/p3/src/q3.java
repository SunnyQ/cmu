...
public class q3 {
  ...
  public static class TokenizerMapper extends Mapper<Object, Text, Text, IntWritable> {
    ...
    public void map(Object key, Text value, Context context) throws IOException,
      ...
      List<String> allOccurance = new ArrayList<String>();
      StringTokenizer itr = new StringTokenizer(value.toString());
      while (itr.hasMoreTokens())
        allOccurance.add(itr.nextToken());

      for (int i = 0; i < allOccurance.size(); i++)
        for (int j = i + 1; j < allOccurance.size(); j++) {
          int first = allOccurance.get(i).compareTo(allOccurance.get(j)) < 0 ? i : j;
          context.write(
                  new Text("(" + allOccurance.get(first) + ", "
                          + allOccurance.get(first == i ? j : i) + ")"), one);
        }
    }
  }
